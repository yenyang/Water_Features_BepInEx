    ...
    public class LocaleEN : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(WaterFeaturesSettings.CreekSpringWater)), "Constant flow rate from  Streams (Modified Constant Rate Water Sources) unaffected by season or weather." },
                { m_Setting.GetOptionLabelLocaleID(nameof(WaterFeaturesSettings.CreekMeanPrecipitationWeight)), "Seasonality" },
                { m_Setting.GetOptionDescLocaleID(nameof(WaterFeaturesSettings.CreekMeanPrecipitationWeight)), "Streams (Modified Constant Rate Water Sources) flowrates will increase by this percentage during the season with the highest mean seasonal precipitation. Other seasons will be proportional." },
                { m_Setting.GetOptionLabelLocaleID(nameof(WaterFeaturesSettings.CreekCurrentPrecipitationWeight)), "Stormwater Effects" },
                { m_Setting.GetOptionDescLocaleID(nameof(WaterFeaturesSettings.CreekCurrentPrecipitationWeight)), "Streams (Modified Constant Rate Water Sources) flowrates will increase by this percentage when current precipitation (rain) is at a maximum. Less precipitation is proportional." },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(WaterFeaturesSettings.TideHeight)), "Tides are the biggest waves and they cause the sea to rise and fall along the shore. Tides can add sandy graphics along shorelines but the sand may not persist the entire time between low tide and high tide. " },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(WaterFeaturesSettings.Damping)), "A higher value makes waves stronger while a lower value makes waves weaker. Stronger waves reach farther into the map. Weak waves may disperse before reaching shore. Vanilla is 99.5%. Recommended 99.7%-99.9%." },
                ...
                { "Assets.NAME[WaterSource Creek]", "Stream - Constant or Variable Rate Water Source" },
                { "Assets.DESCRIPTION[WaterSource Creek]", "Emits water depending on the settings for this mod. With Seasonal Streams disabled, the flow rate will be constant. With Seasonal Streams enabled the flow rate will vary with season, precipitation, and snowmelt depending on your settings. Left click to place within playable area. Hover over and right click to remove." },
                ...
                { "Assets.NAME[WaterSource AutofillingLake]", "Lake - Constant Level Water Source" },
                { "Assets.DESCRIPTION[WaterSource AutofillingLake]", "Fills quickly until it gets to the desired level and then maintains that level. If it has a target elevation below the ground level, it can drain water faster than evaporation. Right click to designate the target elevation. Left click to place within playable area. Hover over and right click to remove." },
